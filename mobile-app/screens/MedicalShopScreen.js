...
const ACTIONS = [
  { status: "Received", label: "Mark as Received", icon: "download-outline", color: "#3B82F6", desc: "Confirm batch arrival at your store" },
  { status: "Available for Sale", label: "Available for Sale", icon: "bag-check-outline", color: "#10B981", desc: "List batch as ready for customers" },
];
...
export default function MedicalShopScreen({ navigation }) {
  ...
  const triggerSuccess = (status, placeName) => {
    setLastStatus(status);
    setLastLocation(placeName);
    setSelectedAction(status);
    successAnim.setValue(0);
    Animated.timing(successAnim, { toValue: 1, duration: 400, useNativeDriver: true }).start();
  };

  // Optimized: Send API with place name, fetch fresh location in background
  const update = async (status) => {
    ...
    try {
      ...
      if (isSuccess) {
        triggerSuccess(status, cachedLocation.place);
        ...
        Alert.alert("Error", res?.data?.error || res?.data?.message || "Update failed");
      }
    } catch (err) {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            <Text style={styles.screenTag}>MEDICAL SHOP PORTAL</Text>
        ...
        <View style={styles.card}>
          <Text style={styles.fieldLabel}>
            <Ionicons name="barcode-outline" size={13} color="#10B981" />{"  "}BATCH ID
          </Text>
        ...
        {/* Action Cards */}
        <Text style={styles.sectionLabel}>SHOP ACTIONS</Text>
...
const styles = StyleSheet.create({
  ...
  orb: {
    ...
    backgroundColor: "rgba(16,185,129,0.06)", top: -60, right: -60,
  },
  ...
  headerText: { flex: 1 },
  screenTag: { color: "#10B981", fontSize: 10, fontWeight: "700", letterSpacing: 1.5 },
  ...
  inputFocused: { borderColor: "rgba(16,185,129,0.5)" },
